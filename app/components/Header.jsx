        ...
        <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm' : 'bg-transparent border-b border-transparent'}`}>
            <div className='flex items-center h-16 px-6 max-w-7xl mx-auto w-full gap-4'>
                ...
                <div className="hidden md:flex gap-6 items-center">
                    <button onClick={() => onScrollToSection('products')} className={`text-sm font-bold tracking-widest uppercase hover:text-[#C4002E] transition-colors ${scrolled ? 'text-black' : 'text-white'}`}>Products</button>
                    <button onClick={() => onScrollToSection('collections')} className={`text-sm font-bold tracking-widest uppercase hover:text-[#C4002E] transition-colors ${scrolled ? 'text-black' : 'text-white'}`}>Collections</button>
                    <button
                        onClick={onOpenAbout}
                        className={`text-sm font-bold tracking-widest uppercase hover:text-[#C4002E] transition-colors ${scrolled ? 'text-black' : 'text-white'}`}
                    >
                        About
                    </button>
                </div>
                ...
                <div className="md:hidden relative">
                    ...
                    <div className={`absolute top-full right-0 w-48 bg-white border border-gray-100 shadow-xl py-2 transition-all duration-200 transform origin-top-right ${mobileMenuOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
                        <button onClick={() => { onScrollToSection('products'); setMobileMenuOpen(false); }} className="w-full text-left px-5 py-3 text-sm font-bold tracking-widest uppercase text-black hover:bg-gray-50 hover:text-[#C4002E] transition-colors">Products</button>
                        <button onClick={() => { onScrollToSection('collections'); setMobileMenuOpen(false); }} className="w-full text-left px-5 py-3 text-sm font-bold tracking-widest uppercase text-black hover:bg-gray-50 hover:text-[#C4002E] transition-colors">Collections</button>
                        <button onClick={() => { onOpenAbout(); setMobileMenuOpen(false); }} className="w-full text-left px-5 py-3 text-sm font-bold tracking-widest uppercase text-black hover:bg-gray-50 hover:text-[#C4002E] transition-colors">About</button>
                    </div>
                </div>
