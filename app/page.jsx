...
function Page() {
    ...
    const firstUrl = (images) => {
        ...
        if (Array.isArray(images)) {
            ...
            if (head && typeof head === "object") return head.url || ""
            return ""
        }
        if (typeof images === "object") return images.url || ""
        return ""
    }
    ...
    useEffect(() => {
        const run = async () => {
            ...
            try {
                ...
                const prod = (Array.isArray(d1) ? d1 : []).filter(
                    (x) => String(x?.type || "product").toLowerCase() === "product"
                )
                setItems(prod)

                const cols = (Array.isArray(d1) ? d1 : []).filter(
                    (x) => String(x?.type || "collection").toLowerCase() === "collection"
                )
                setCollections(cols)
            } catch (e) {
                console.error(e)
    ...
    const heroImg = useMemo(() => {
        const url = firstUrl(activeCollectionObj?.images)
        return heroSize(url)
    }, [activeCollectionObj])

    const filtered = useMemo(() => {
        const list = Array.isArray(products) ? products : []
        if (!activeColl) return list
        return list.filter((it) => (it.colls || "") === activeColl)
    }, [products, activeColl])

    const colNames = useMemo(
        () =>
            Array.from(
                new Set((collections || []).map((c) => c?.name).filter(Boolean))
            ),
        [collections]
    )
    ...
    return (
        <div>
            <nav className="fixed inset-x-0 top-0 z-40 flex justify-between items-center w-full p-4 bg-black">
                <button aria-label="home">
                    <svg width="86" height="32" viewBox="0 0 86 32" className="fill-white" xmlns="http://www.w3.org/2000/svg">
                ...
                >
                    <div className="flex gap-3 p-2 hover:bg-white hover:text-black">
                        <span>{activeColl || activeCollectionObj?.name || "All collections"}</span>
                        <svg width="24" height="25" viewBox="0 0 24 25" className="fill-current" xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd" clipRule="evenodd" d="M6 7.5L12 13.5L18 7.5L20 9.5L12 17.5L4 9.5L6 7.5Z" />
                        </svg>
                    </div>
                </div>

                <button className="rounded-full p-2 hover:bg-white group" aria-label="search">
                    <svg width="32" height="32" viewBox="0 0 32 32" className="fill-white group-hover:fill-black" xmlns="http://www.w3.org/2000/svg">
            ...
            <main className="">
                <div className="w-full h-[calc(100vh-0rem)]">
                    {heroImg ? (
                        <img
                            src={heroImg || ""}
                ...
                <section className="container mx-auto py-6 px-6 md:px-32 lg:px-48">
                    {loading ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {filtered.map((it) => {
                                const first = firstUrl(it.images)
                                const imgSrc = first ? thumb(first) : ""
                                ...
                                return (
                                    <div key={it.id} className="text-center">
                                        <div className="bg-gray-100">
                                            ...
                                            <a
                                                href={it.url}
                                                className="block bg-black text-white text-center text-lg py-3 w-full cursor-pointer"
                                            >
                                                Shop now
                                            </a>
                                        </div>

                                        <h3 className="mt-2">{it.name}</h3>
                                        {it.price !== "" && it.price != null && (
                                            <span>{Number(it.price)} Bath</span>
                                        )}
                                    </div>
                                )
                            })}
                            {filtered.length === 0 && (
                                <div className="col-span-full text-center text-gray-500">
                                    No products{activeColl ? ` in "${activeColl}"` : ""}.
                                </div>
                            )}
                        </div>
                    )}
            ...
            {collectionDrop && (
                <div className="fixed top-16 left-0 right-0 z-50 flex justify-center text-white">
                    <div className="bg-black overflow-hidden min-w-[240px]">
                        <button
                            onClick={() => { setActiveColl(""); setCollectionDrop(false) }}
